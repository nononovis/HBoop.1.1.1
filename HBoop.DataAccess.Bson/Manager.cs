...
namespace HBoop.DataAccess.Bson
{
...

    {
...

        {
            ...
            Collection = Database<T>.GetCollection();
...

        {
...

            {
                ...
                var update = Updater.Combine(updates).CurrentDate(i => i.ModifiedDate);

                return Collection.UpdateMany(filter, update.CurrentDate(i => i.ModifiedDate)).IsAcknowledged;
...

        {
            ...
            return RetryPolicy.Handle<MongoConnectionException>(i => i.InnerException.GetType() == typeof(IOException))
